...
class XcodeProject(PBXDict):
    ...
    def add_folder(self, os_path, parent=None, excludes=None, recursive=True, create_build_files=True):
        ...
        for (grp_path, subdirs, files) in os.walk(os_path):
            ...
            for path, kwds in add_files:
                kwds.pop('name', None)
                self.add_file_if_doesnt_exist(path, **kwds)
    ...
    def add_file_if_doesnt_exist(self, f_path, parent=None, tree='SOURCE_ROOT', create_build_files=True, weak=False, ignore_unknown_type=False, force_arc_compile=False):
    ...
    def add_file(self, f_path, parent=None, tree='SOURCE_ROOT', create_build_files=True, weak=False, ignore_unknown_type=False, force_arc_compile=False):
        results = []
        abs_path = ''
        ...
        #syslog.syslog( syslog.LOG_ALERT, 'adding: ' + f_path )
