...
public class GameCircleSocial : ISocialPlatform {
    ...
    //------------------------------------------------------------------------------------------------------------------------------
    private Action<bool>  userAuthenticationCallback;

    public void Authenticate(ILocalUser user, System.Action<bool> callback)
    {
        //Debug.Log("###-XXX-### GameCircleSocial - Authenticate");
        // Forward the AGSClient callbacks to the passed in callback.
        userAuthenticationCallback = callback;
        authenticationCallback = Authenticated;
        // If using GameCircle with the Unity Social API,
        // initialize it with leaderboards and achievements, but not whispersync.
        AGSClient.Init(/*Leaderboards*/true,/*Achievements*/true,/*Whispersync*/false);
    }

    private void Authenticated(bool success)
    {
        //Debug.Log("###-XXX-### GameCircleSocial - Authenticated - success: " + success);
        //Authenticated correctly, grab user data
        if (success)
            RequestLocalPlayer(userAuthenticationCallback);
        else //bubble failure up to user
            userAuthenticationCallback(success);
    }
    //------------------------------------------------------------------------------------------------------------------------------
