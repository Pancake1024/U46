...
- (BOOL)verifyRestoredPurchase:(SKPaymentTransaction *)transaction withDelegate:(NSObject<RRVerificationControllerDelegate> *)verificationDelegate error:(out NSError **)outError;
{
	if ((transaction.transactionState == SKPaymentTransactionStatePurchased) || (transaction.transactionState == SKPaymentTransactionStateRestored)) {
		RRTransactionVerifier *verifier = [[RRTransactionVerifier alloc] initWithRestoredPurchase:transaction delegate:verificationDelegate controller:self];
		[verificationsInProgress addObject:verifier];
...
{
	[verifier.delegate verificationControllerDidVerifyPurchase:verifier.transaction wasRestored:verifier.restored isValid:isValid];
