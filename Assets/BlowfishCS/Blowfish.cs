...
namespace BlowFishCS
{
    class BlowFish
    {
        ...
        static string GetASCIIString(byte[] b)
        {
...
#if UNITY_WP8
            byte[] bytes = Decrypt_CBC(HexToByte(ct.Substring(16)));
        ...
        public string Encrypt_ECB(string pt)
        {
...
#if UNITY_WP8
            {
                if (0 == pt.Length)
                    ...
                    return ByteToHex(Encrypt_ECB(GetASCIIBytes(pt)));
            }
#else
        ...
        public string Decrypt_ECB(string ct)
        {
...
#if UNITY_WP8
            {
                ...
                byte[] tmp = Decrypt_ECB(HexToByte(ct));
                if (0 == tmp.Length)
                    return string.Empty;
                else
                    return GetASCIIString(tmp).Replace("\0", "");//Encoding.UTF7.GetString(tmp).Replace("\0", "");
            }
#else
